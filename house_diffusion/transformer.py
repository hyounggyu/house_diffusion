...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, door_mask, self_mask, gen_mask):
        assert (
            gen_mask.max() == 1 and gen_mask.min() == 0
        ), f"{gen_mask.max()}, {gen_mask.min()}"
        x2 = self.norm_1(x)
        x = (
            x
            + self.dropout(self.door_attn(x2, x2, x2, door_mask))
...
class TransformerModel(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_layers = nn.ModuleList(
            [
                EncoderLayer(self.model_channels, 4, 0.1, self.activation)
        ...
        if not self.analog_bit:
            self.output_linear_bin1 = nn.Linear(
                162 + self.model_channels, self.model_channels
            )
            self.output_linear_bin2 = EncoderLayer(
                self.model_channels, 1, 0.1, self.activation
            )
            self.output_linear_bin3 = EncoderLayer(
                self.model_channels, 1, 0.1, self.activation
            )
            self.output_linear_bin4 = nn.Linear(self.model_channels, 16)

        print(
            f"Number of model parameters: {sum(p.numel() for p in self.parameters() if p.requires_grad)}"
        )
    ...
    def create_image(self, points, connections, room_indices, img_size=256, res=200):
        ...
        new_room_indices = room_indices.unsqueeze(1).expand(
            [p2.shape[0], res, p2.shape[1], 1]
        )
        ...
        img[
            th.arange(xs.shape[0])[:, None],
            :,
            xs.view(img.shape[0], -1),
            ys.view(img.shape[0], -1),
        ] = new_room_indices.reshape(img.shape[0], -1, 1).float()
        return img.detach()
    ...
    def forward(self, x, timesteps, xtalpha, epsalpha, is_syn=False, **kwargs):
        ...
        if not self.analog_bit:
            ...
            out_bin = self.output_linear_bin2(
                out_bin,
                kwargs[f"{prefix}door_mask"],
                kwargs[f"{prefix}self_mask"],
                kwargs[f"{prefix}gen_mask"],
            )
            out_bin = self.output_linear_bin3(
                out_bin,
                kwargs[f"{prefix}door_mask"],
                kwargs[f"{prefix}self_mask"],
                kwargs[f"{prefix}gen_mask"],
            ...
            out_bin = out_bin.permute(
                [0, 2, 1]
            )  # -> convert back [N x S x C] to [N x C x S]

        out_dec = out_dec.permute(
            [0, 2, 1]
        )  # -> convert back [N x S x C] to [N x C x S]
