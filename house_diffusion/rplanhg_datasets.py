...
class RPlanhgDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False):
        ...
        if self.set_name == "eval":
            cnumber_dist = np.load(
                f"processed_rplan/rplan_train_{target_set}_cndist.npz",
                allow_pickle=True,
            )["cnumber_dist"].item()
    ...
    def build_graph(self, rms_type, fp_eds, eds_to_rms, out_size=64):
        ...
        # get rooms masks
        eds_to_rms_tmp = []
        for l in range(len(eds_to_rms)):  # noqa
        ...
        for k in range(len(nodes)):
            # add rooms and doors
            eds = []
            for l, e_map in enumerate(eds_to_rms_tmp):  # noqa
                if k in e_map:
                    eds.append(l)
            # draw rooms
            ...
            for eds_poly in [eds]:
                poly = self.make_sequence(np.array([fp_eds[l][:4] for l in eds_poly]))  # noqa
                poly = poly[0]
